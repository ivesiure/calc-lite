...
function Atan(h) {
  var g = AngOut(
    Div(Mult(I, Ln(Div(Sub(One, Mult(I, h)), Add(One, Mult(I, h))))), Two)
  );
  if (h.i == 0) {
    g.i = 0;
  }
  ...
  if (SModeValue == 1) {
    for (i = 1; i <= DispHgt; i++) {
      if (expList[g - i + 1] != '') {
        ...
        JGetId('ecalc_stack' + i.toString()).style.fontSize =
          newFontSize + 'px';
        ...
        JGetId('ecalc_stack' + (i + 1).toString()).style.fontSize =
          newFontSize + 'px';
  ...
  try {
    ...
    for (k = 0; k < val.length; k++) {
      ...
      for (jsk = 0; jsk < JSmath.length; jsk++) {
        if (
          val.indexOf(JSmath[jsk], k) == k &&
          val.indexOf('Math.', k - 5) != k - 5
        ) {
...
function Infix2RPN(u) {
  ...
  u = UpdateStr(
    u,
    "(z=='i')&(!isLet(prevz))&((!isLet(nextz))|(i==(lastz)))",
    '(0,1)'
  );
  u = UpdateStr(
    u,
    "((prevz==')')|isrDigit(prevz))&((z=='(')|( (isFnc(z))&(z!='!')&(z!='e')))",
    '*z'
  );
  ...
  for (m = 0; m < u.length; m++) {
    ...
    if (v == '.') {
      for (j = g.length; j >= 0; j--) {
        ...
        if (
          (g.charAt(j) == 'e') | !isDigit(g.charAt(j)) &&
          !isOp(g.charAt(j))
        ) {
...
function evalSolverNums() {
  ...
  for (iSlv = 0; iSlv < solverSize * solverSize; iSlv++) {
...
function stackScrollUp() {
  return setInterval(
    'if((lpos>0)&&(lpos<=lmax)){lpos--};UpdateDisp(lpos)',
    150
  );
}
...
constArray[1] = new Array(
  'Cc',
  'Coulomb Constant',
  '8.9875517873682e9',
  'Nm&sup2;/coul&sup2;'
);
constArray[2] = new Array(
  'g',
  'Acceleration of gravity',
  '9.80665',
  'm/s&sup2;'
);
constArray[3] = new Array(
  'G',
...
constArray[6] = new Array(
  'F',
  'Faraday Constant',
  '9.64853383e4',
  'Cmol<sup class="constSuper">&nbsp;-1</sup>'
);
...
constArray[10] = new Array(
  'Na',
  'Avogadros Number',
  '6.02214179e23',
  'mol<sup class="constSuper">&nbsp;-1</sup>'
);
...
constArray[14] = new Array(
  'Rdb',
  'Rydberg constant',
  '1.097373153413e7',
  'm<sup class="constSuper">&nbsp;-1</sup>'
);
constArray[15] = new Array('Vm', 'Molar Volume', '2.241409e-2', 'm&sup3;/mol');
constArray[16] = new Array(
  '&epsilon;0',
  'Permittivity of a Vacuum',
  '8.8541878176204e-12',
  'F/m'
);
constArray[17] = new Array(
  '&sigma;',
  'Stefan-Boltzmann Constant',
  '5.6705119e-8',
  'W/m&sup2;/K^4'
);
constArray[18] = new Array(
  '&phi;',
  'Magnetic Flux Quantum',
  '2.0678346161e-15',
  'Wb'
);
constArray[19] = new Array(
  '&mu;o',
  'Permeability of a Vacuum',
  '1.2566370614359e-6',
  'N/A&sup2;'
);
constArray[20] = new Array(
  '&mu;b',
  'Bohr Magneton',
  '9.2740154e-24',
  'm&sup2;/Wb'
...
function updateBases(g) {
  if (g == 'dec_box') {
    ...
    if (decValue >= 0) {
      document.getElementById('hex_box').value = base(
        decValue,
        '0123456789ABCDEF'
      );
...
function constLoad() {
  if (constState == 0) {
    for (var g = 0; g <= 11; g++) {
      document.getElementById('CNSTAbv' + g.toString()).innerHTML =
        constArray[g][0];
      document.getElementById('CNSTDesc' + g.toString()).innerHTML =
        constArray[g][1];
    ...
    if (constState == 1) {
      for (g = 0; g <= 11; g++) {
        ...
        document.getElementById('CNSTDesc' + g.toString()).innerHTML =
          constArray[g + 12][1];
        document.getElementById('CNSTDesc' + g.toString()).style.width =
          '220px';
      }
...
function addLi(m, w, q) {
  ...
  document.getElementById('solverWrap').style.width =
    ((w + q) * u + 14).toString() + 'px';
  document.getElementById('solverBracketsWrap').style.width =
    ((w + q) * u + 32).toString() + 'px';
  document.getElementById('solverWrap').style.height = v.toString() + 'px';
  document.getElementById('solverBracketsWrap').style.height =
    (v + 18).toString() + 'px';
  for (i = 0; i <= w * q - 1; i++) {
...
function saveRootValues() {
  for (i = 0; i <= solverSize; i++) {
    solverArrayRoot[i] = document.getElementById(
      'rootCoeff' + i.toString()
    ).value;
  }
}
...
function insertD() {
  document.getElementById('ecalc_body').innerHTML +=
    "<div id='ecalc_demo'><div id='ecalcBuy' onmouseover='windowButtonHover(this);' onmouseout='windowButtonOut(this);' onmousedown='buyEcalc();'></div><div id='ecalcCont' class='ecalcContOut' onmouseover='windowButtonHover(this);' onmouseout='windowButtonOut(this);' onmousedown='restD();'></div></div>";
}
var req;
      ...
      if (unitCat == 9) {
        for (i = 0; i <= 11; i++) {
          if (unitType[unitCat][i] != undefined) {
            if (i != g) {
              unitStore[unitCat][i] = formatNumNice(
                tempConvt(
                  parseFloat(h),
                  unitType[unitCat][g][3],
                  unitType[unitCat][i][3]
                ),
                12
              );
            } else {
        ...
        for (i = 0; i <= 11; i++) {
          if (unitType[unitCat][i] != undefined) {
            if (i != g) {
              unitStore[unitCat][i] = formatNumNice(
                parseFloat(
                  (unitType[unitCat][g][3] / unitType[unitCat][i][3]) * h
                ),
                12
              );
            } else {
...
function unitsFormat(h) {
  ...
  if (g != '' && fromNumPress == false) {
    document.getElementById('unitInput' + h).value = formatNumUC(
      parseFloat(g),
      7
    );
  }
...
function updateUnits(h) {
  ...
  document.getElementById('unitTitle').innerHTML =
    'Unit Conversion:&nbsp;' + unitTitleText[h];
}
...
function smallColorBackgroundSet(index) {
  for (i = 1; i <= 6; i++) {
    document.getElementById('smallColor' + i.toString()).style.backgroundColor =
      eval('color' + i + '_2');
  }
  document.getElementById('smallColor' + index).style.backgroundColor = eval(
    'color' + index + '_1'
  );
}
...
function unitsBgColor(g) {
  ...
  for (i = 0; i <= 11; i++) {
    if (unitType[unitCat][i] != undefined && i != g) {
      ...
      document.getElementById('unitInput' + i).style.backgroundColor =
        '#FFFFFF';
...
function insertAtCursor(l) {
  ...
  if (myField.selectionStart || myField.selectionStart == '0') {
    ...
    myField.value =
      myField.value.substring(0, h) +
      l +
      myField.value.substring(g, myField.value.length);
    myField.focus();
...
function tryHidingKeyboard() {
  navigator.virtualKeyboard.hide();
  var field = document.createElement('input');
  field.setAttribute('type', 'button');
  field.setAttribute('style', 'position: absolute;left:-200');
...
function backAtCursor() {
  ...
  if (myField.selectionStart || myField.selectionStart == '0') {
    ...
    myField.value =
      myField.value.substring(0, l - 1) +
      myField.value.substring(h, myField.value.length);
...
function openPal() {
  var ecalcBody = document.getElementById('ecalcBody');
  if (ecalcBody) {
    document.getElementById('ecalcBody').style.width = '100%';
  }
